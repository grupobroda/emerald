            ...
            {INFO_MOCK.map((info)=> {
                return(
                    <Link href={info.url} key={info.titulo} className="relative flex flex-col items-center text-center py-30 gap-10 bg-white p-10 rounded-lg bg-center bg-cover bg-no-repeat group cursor-pointer translateTransition" style={{backgroundImage: `url(${info.imagen})`}}>
                        ...
                        <button className="relative flex items-center justify-center text-white bg-neutral-300/15 border p-3 rounded-full z-10 cursor-pointer transition-[width] duration-300 overflow-hidden w-12 group-hover:w-48 group-hover:bg-white group-hover:text-black group-hover:border-transparent">
                            <span className="absolute whitespace-nowrap opacity-0 group-hover:relative group-hover:opacity-100 transition-opacity duration-300">
                                Más información
                            </span>
                            <ChevronRight  />
                        </button>
